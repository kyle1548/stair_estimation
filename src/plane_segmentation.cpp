...
void PlaneSegmentation::setInputCloud(pcl::PointCloud<PointT>::Ptr cloud) {
    ...
    try {
        if (tf_buffer_->canTransform("map", cloud->header.frame_id, tf2::TimePointZero, tf2::durationFromSec(0.0))) {
            geometry_msgs::msg::TransformStamped tf_msg = 
                tf_buffer_->lookupTransform("map", cloud->header.frame_id, tf2::TimePointZero);
            Eigen::Affine3d eigen_transform = tf2::transformToEigen(tf_msg.transform);
            pcl::transformPointCloud(*cloud, *cloud, eigen_transform);
        } else {
            RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000, 
                "Cannot transform cloud from %s to map. TF not ready.", cloud->header.frame_id.c_str());
        }
